...
module Banner = {
  ...
  let make = (~data, ~onClose=?) => {
    ...
    let closeBtn = switch onClose {
    | None => React.null
    | Some(onClose) =>
      <View style={styles["right_button"]}>
        <IconButton icon=Icons.Close.build onPress={_ => onClose(_ => Unchecked)} />
      </View>
    }
    ...
    <View
      style={
        open Style
        array([style(~backgroundColor=theme.colors.textPrimary, ()), styles["banner"]])
      }>
      <View
        style={
          open Style
...
module Error = {
  ...
  let make = (~msg, ~logMsg, ~logToast, ~checkForUpdates, ~setStep) => {
    ...
    let data =
      <>
        <Icons.Info size=30. style={styles["banner_icon"]} color=theme.colors.textHighEmphasis />
        <Typography.Body1 style={styles["banner_text_with_icon"]}>
          {msg->React.string}
        </Typography.Body1>
        <Buttons.FormPrimary text=I18n.Btn.try_again onPress=check style={styles["flex_elt"]} />
      </>

    <> <CheckButton checkForUpdates /> <Banner data onClose={_ => setStep(_ => Unchecked)} /> </>
  }
}
  ...
  let make = (~checkForUpdates, ~download, ~setStep) => {
    ...
    let data =
      <>
        ...
        <Buttons.FormPrimary
          text=I18n.Btn.download onPress={_ => download()} style={styles["flex_elt"]}
        />
      </>
    <> <CheckButton checkForUpdates /> <Banner data onClose={_ => setStep(_ => Unchecked)} /> </>
  }
...
module DownloadComplete = {
  ...
  let make = (~checkForUpdates, ~setStep) => {
    ...
    let data =
      <>
        <Icons.CheckFill size=22. color=theme.colors.valid style={styles["banner_icon"]} />
        <Typography.Body1 style={styles["banner_text_with_icon"]}>
...
let make = () => {
  ...
  React.useEffect0(() => {
    ...
    IPC.on("download-progress", (_, percent) => setStep(_ => Downloading(percent)))
    IPC.on("update-error", (_, msg) => setStep(_ => Error(msg)))
    IPC.on("check-update-error", (_, msg) => setStep(_ => CheckUpdateError(msg)))
    None
  })
  ...
  let (autoUpdates, setCheckUpdates) = React.useState(() => configFile.autoUpdates)
  ...
  let writeCheckUpdates = f => {
    ...
    writeConf(c => {...c, autoUpdates: autoUpdates})
  }
  <>
    <Block title=I18n.Settings.app_update>
      <View style={Style.style(~width="100%"->StyleUtils.stringToSize, ())}>
        <View style={Style.style(~textAlign=#center, ())}>
          ...
          | Downloading(percentage) => <Downloading percentage checkForUpdates />
          ...
          | CheckUpdateError(logMsg) =>
            ...
            <Error msg logMsg logToast checkForUpdates setStep />
          | UpdateAvailable => <UpdateAvailable checkForUpdates download setStep />
