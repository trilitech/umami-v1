...
let make = (
  ...
  let data =
    contract.tokens->Map.Int.map(x =>
      switch x {
      | (Partial(_, _, _), _) => React.null
      | (Full(nft), balance) =>
        <NftRowToken
          account address=contract.address balance nft selected setSelected hidden setHidden
        />
      }
    )
  ...
  let header =
    <View
      ...
      <Typography.Notice style={styles["iconButton"]}>
        {contract.name->Option.getWithDefault((contract.address :> string))->React.string}
      </Typography.Notice>
      ...
      <IconButton
        ...
        tooltip=("open_in_explorer" ++ (contract.address :> string), I18n.Tooltip.open_in_explorer)
        onPress={_ =>
          switch Network.externalExplorer(config.network.chain) {
          | Ok(url) => System.openExternal(url ++ (contract.address :> string))
