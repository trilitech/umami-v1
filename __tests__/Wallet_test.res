...
let testkey1 = "tz1ckaMTzEzzgYrPqXY4Y6dMxV38qf3LKJ5p"->PublicKeyHash.build->Result.getExn

let testkey2 = "tz1iAmRoLmqhv4WekE4KkLjCUuv22Xq2MFoF"->PublicKeyHash.build->Result.getExn

let testkey3 = "tz1LboVmDa9YdVR8S2LozRRoQkd26WFd6Dd8"->PublicKeyHash.build->Result.getExn
...
let testkey5 = "tz1UBwnNSFrvbPmtVT4vHCsba3mjMGtKhpu2"->PublicKeyHash.build->Result.getExn
...
let futureEqual = ({expectEqual, callback}: Types.testAsyncUtils<_>, eqcomp, fn) =>
  before()
  ->bind(fn)
  ->bind(v => after()->map(() => v))
  ->Promise.get(v => {
...
describe("Wallet", ({testAsync}) => {
  ...
  testAsync("Add alias", test =>
    test->futureEqual(Ok(Some(testkey1)), () =>
      add(~config, ~alias="testkey1", ~address=testkey1)->bind(_ =>
        getAddressForAlias(~config, ~alias="testkey1")
      )
    )
  )
  ...
  testAsync("Get from unknown alias", test =>
    test->futureEqual(Ok(None), () =>
      add(~config, ~alias="testkey1", ~address=testkey1)->bind(_ =>
        getAddressForAlias(~config, ~alias="testkey2")
      )
    )
  )
  ...
  testAsync("Get from address", test =>
    test->futureEqual(Ok(Some("testkey1")), () =>
      add(~config, ~alias="testkey1", ~address=testkey1)->bind(_ =>
        getAliasForAddress(~config, ~address=testkey1)
      )
    )
  )
  ...
  testAsync("Get from unknown address", test =>
    test->futureEqual(Ok(None), () =>
      add(~config, ~alias="testkey1", ~address=testkey1)->bind(_ =>
        getAliasForAddress(~config, ~address=testkey2)
      )
    )
  )
  ...
  let getResult = [("testkey1", testkey1), ("testkey2", testkey2), ("testkey3", testkey3)]
  ...
  testAsync("Get all", test =>
    test->futureEqual(Ok(getResult), () =>
      ...
      ->bind(_ => get(~config))
      ->map(a => a->Array.map(({Alias.name: name, address, _}) => (name, address)))
      ->map(a => a->SortArray.stableSortBy(((v1, _), (v2, _)) => compare(v1, v2)))
    )
  )
  ...
  testAsync("LegacyImport", test =>
    test->futureEqual(Ok(testkey5), () =>
      HDWalletAPI.Accounts.legacyImport(~config, "zebra", recoveryPhrase5, ~password="totototo")
    )
  )
})
...
describe("Signer detection", ({test}) => {
  ...
  sks->List.forEachWithIndex((i, (inp, out)) =>
    test("Parsing prefix n" ++ i->Int.toString, ({expectEqual}) => {
  ...
  let sk1pkh = "tz1gr5TA8waD7LcrXNRaSz7Bys2Y14AWZnGH"->PublicKeyHash.build->Result.getExn

  test("Parsing SK Ledger", ({expectEqual}) =>
    expectEqual(Ok(skl1Res), Decode.fromSecretKey(skl1NoPref, ~ledgerBasePkh=sk1pkh))
  )
})
