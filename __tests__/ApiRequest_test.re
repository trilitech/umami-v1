    ...
    let mockFuture =
      Umami.Promise.make(resolve => {
        Js.Global.setTimeout(
          ...
          () => {
            ReactTestingLibrary.act(() => {resolve(Ok(requestResult))})
          },
          1,
        )
      });
...
describe("useGetter", () => {
  ...
  test(
    ...
    ApiRequest.NotAsked,
    [Loading(None), Done(Ok(Mock.requestResult), ApiRequest.initCache())],
  );
  test(
    ...
    [
      Loading(Some("something")),
      Done(Ok(Mock.requestResult), ApiRequest.initCache()),
    ],
  );
  test(
...
describe("useLoader", () => {
  let useCallF = init => {
    ...
    let state =
      ApiRequest.useLoader(~get=Mock.get, ~kind=Account, ~requestState, ());
